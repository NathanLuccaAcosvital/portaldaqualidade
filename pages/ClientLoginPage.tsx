...
const ClientLoginPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen w-full flex bg-[#040a1d] relative selection:bg-blue-100 overflow-x-hidden font-sans">
      ...
      <aside className="hidden lg:flex lg:w-[45%] xl:w-[55%] relative overflow-hidden h-screen shrink-0 border-r border-white/5">
        <LoginHero />
      </aside>

      {/* Login Form Section: Full height flex container */}
      <main className="flex-1 min-h-screen flex flex-col items-center justify-center p-6 md:p-12 lg:p-16 xl:p-24 bg-white lg:rounded-l-[3.5rem] relative z-10 shadow-[-20px_0_60px_rgba(0,0,0,0.2)] lg:shadow-[-40px_0_100px_rgba(0,0,0,0.4)]">
        
        {/* Language Switcher - Responsive positioning */}
        <div className="absolute top-6 right-6 md:top-10 md:right-10 z-50 animate-in fade-in duration-1000">
          <LanguageSelector />
        </div>

        {/* Login Container: Max width restricted for elegance */}
        <div className="w-full max-w-[380px] sm:max-w-[400px] animate-in zoom-in-95 duration-700">
          <div className="space-y-8 md:space-y-12">
            <div className="flex justify-center lg:hidden mb-10">
               <img src="https://wtydnzqianhahiiasows.supabase.co/storage/v1/object/public/public_assets/hero/logo.png" alt="Aços Vital" className="h-12 object-contain" />
            </div>
            ...
            <footer className="pt-8 md:pt-10 border-t border-slate-100 flex items-center justify-between">
               <div className="space-y-0.5 md:space-y-1">
                  <p className="text-[9px] md:text-[10px] font-black text-slate-900 uppercase tracking-[2px]">Aços Vital S.A.</p>
                  <p className="text-[8px] md:text-[9px] font-bold text-slate-500 uppercase tracking-widest">Sistemas de Gestão Industrial</p>
               </div>
               <img 
                 src="https://wtydnzqianhahiiasows.supabase.co/storage/v1/object/public/public_assets/hero/logo.png" 
                 alt="" 
                 className="h-4 md:h-5 opacity-30 grayscale hidden sm:block" 
                 aria-hidden="true" 
               />
            </footer>
          </div>

...
const InternalLoginPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex bg-white font-sans overflow-hidden">
      ...
      <div className="hidden lg:flex lg:w-[60%] relative bg-[#081437]">
        ...
        <div className="relative z-10 flex flex-col justify-between p-20 w-full text-white">
          <div className="flex flex-col gap-12">
            <Link to="/login" className="inline-flex items-center gap-2 text-slate-400 hover:text-white transition-colors group">
              ...
              <span className="text-[10px] font-black uppercase tracking-[4px]">Voltar ao Portal Público</span>
            </Link>
            <img src={LOGO_URL} alt="Aços Vital" className="h-16 self-start drop-shadow-2xl" />
          </div>
          
          <div className="space-y-6">
            <div className="h-[2px] w-12 bg-blue-500" />
            <h1 className="text-6xl font-black tracking-tighter leading-tight">Staff Portal<br/><span className="text-blue-400">Control Center.</span></h1>
            <p className="text-xl text-slate-400 font-medium max-w-xl leading-relaxed">Ambiente restrito para auditores técnicos e administradores globais da infraestrutura Aços Vital.</p>
          </div>
          <p className="text-[10px] font-black uppercase tracking-[5px] text-slate-500">Intranets & Governance v2.4</p>
        </div>
      </div>

      {/* Form de Login Staff Lado Direito */}
      <main className="w-full lg:flex-1 flex items-center justify-center p-8 bg-white">
        <div className="w-full max-w-[420px] p-1 bg-gradient-to-br from-slate-100 to-transparent rounded-3xl">
          <div className="bg-white rounded-[1.4rem] p-10 space-y-10 shadow-sm border border-slate-50">
            <header className="space-y-2">
              <div className="flex items-center gap-2 text-blue-600 mb-2">
                  <ShieldAlert size={18} />
                  <span className="text-[9px] font-black uppercase tracking-widest">Acesso Restrito Staff</span>
              </div>
              <h2 className="text-3xl font-black text-slate-900 tracking-tighter">Entrar agora.</h2>
              <p className="text-slate-400 text-sm font-medium">Use sua identidade corporativa Aços Vital.</p>
            </header>

            <form onSubmit={handleLogin} className="space-y-6">
              <StaffInputField label="ID Corporativo (E-mail)" icon={Mail} type="email" value={credentials.email} onChange={v => setCredentials({...credentials, email: v})} placeholder="seu.nome@acosvital.com" />
              ...
              {error && (
                <div className="p-3 bg-orange-50 text-orange-700 text-[10px] font-bold rounded-xl border border-orange-100 flex items-center gap-2">
              ...
              <button disabled={isLoading} className="w-full bg-[#081437] hover:bg-slate-800 text-white font-black h-14 rounded-xl shadow-xl flex items-center justify-center gap-4 transition-all active:scale-95 disabled:opacity-50">
                 {isLoading ? <Loader2 size={20} className="animate-spin" /> : <><span className="uppercase tracking-[4px] text-[10px]">Autenticar Identidade</span><ArrowRight size={18} className="text-blue-500" /></>}
              </button>
            </form>
...
const StaffInputField = ({ label, icon: Icon, value, onChange, type, placeholder, showToggle, onToggle, isToggled }: any) => (
  <div className="space-y-2">
    ...
    <div className="flex items-center bg-slate-50 border-b-2 border-slate-100 focus-within:border-blue-500 focus-within:bg-white transition-all overflow-hidden">
       <div className="w-10 h-12 flex items-center justify-center text-slate-300"><Icon size={16} /></div>
       <input type={type} required className="flex-1 px-3 py-3 bg-transparent outline-none text-xs font-bold text-slate-800" placeholder={placeholder} value={value} onChange={e => onChange(e.target.value)} />
       {showToggle && <button type="button" onClick={onToggle} className="px-3 text-slate-300 hover:text-blue-500">{isToggled ? <EyeOff size={16} /> : <Eye size={16} />}</button>}
    </div>
  </div>
);

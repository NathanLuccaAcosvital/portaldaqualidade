          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
            ...
            {isFullApproved ? (
              ...
              <>
                ...
                {/* STEP 3: FÍSICO */}
                <section className={`space-y-6 ${currentFile?.metadata?.status !== QualityStatus.APPROVED ? 'opacity-30 pointer-events-none' : ''}`}>
                  <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-[3px] flex items-center gap-2">
                  ...
                  <div className="bg-slate-50 p-6 rounded-[2rem] border border-slate-100 space-y-4">
                    {rejectMode === 'PHYSICAL' ? (
                      ...
                      <div className="space-y-4">
                        ...
                        {user?.role === UserRole.CLIENT && currentFile?.metadata?.status === QualityStatus.APPROVED && !currentFile?.metadata?.physicalStatus && (
                          <div className="grid grid-cols-2 gap-3">
